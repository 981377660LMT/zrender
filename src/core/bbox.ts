...
export function fromLine(x0: number, y0: number, x1: number, y1: number, min: vec2.VectorArray, max: vec2.VectorArray) {
  min[0] = mathMin(x0, x1)
  min[1] = mathMin(y0, y1)
  max[0] = mathMax(x0, x1)
  max[1] = mathMax(y0, y1)
...
export function fromQuadratic(
  ...
  const tx = mathMax(mathMin(quadraticExtremum(x0, x1, x2), 1), 0)
  const ty = mathMax(mathMin(quadraticExtremum(y0, y1, y2), 1), 0)
