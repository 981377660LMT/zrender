...
const firefoxNotSupportOffsetXY =
  env.browser.firefox &&
...
function calculateZrXY(el: HTMLElement, e: ZRRawEvent, out: { zrX?: number; zrY?: number }) {
...
export function getNativeEvent(e: ZRRawEvent): ZRRawEvent {
  return e || (window.event as any) // For IE
}
 ...
 */
export function normalizeEvent(el: HTMLElement, e: ZRRawEvent, calculate?: boolean) {
  e = getNativeEvent(e)
  ...
  if (!isTouch) {
    ...
    const touch = eventType !== 'touchend' ? (<TouchEvent>e).targetTouches[0] : (<TouchEvent>e).changedTouches[0]
