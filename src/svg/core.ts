...
export function getCssString(
  ...
  const selectors = map(keys(selectorNodes), className => {
    return (
      className +
      bracketBegin +
      map(keys(selectorNodes[className]), attrName => {
  ...
  const animations = map(keys(animationNodes), animationName => {
    return (
      `@keyframes ${animationName}${bracketBegin}` +
      map(keys(animationNodes[animationName]), percent => {
        return (
          percent +
          bracketBegin +
          map(keys(animationNodes[animationName][percent]), attrName => {
