...
function createAttrsConvert(desc: ShapeMapDesc): ConvertShapeToAttr {
  const normalizedDesc: [string, string][] = map(desc, item => (typeof item === 'string' ? [item, item] : item))
...
export function brushSVGTSpan(el: TSpan, scope: BrushScope) {
  ...
  const textAlign = TEXT_ALIGN_TO_ANCHOR[style.textAlign as keyof typeof TEXT_ALIGN_TO_ANCHOR] || style.textAlign
...
export function setGradient(style: PathStyleProps, attrs: SVGVNodeAttrs, target: 'fill' | 'stroke', scope: BrushScope) {
...
export function setPattern(el: Displayable, attrs: SVGVNodeAttrs, target: 'fill' | 'stroke', scope: BrushScope) {
...
export function setClipPath(clipPath: Path, attrs: SVGVNodeAttrs, scope: BrushScope) {
