...
export default class CanvasPainter implements PainterBase {
  ...
  /** 并不是所有的使用场景都适合多层 canvas 。微信小程序中没有DOM，而canvas也是一个独立的组件，因此，只能使用 singleCanvas。 */
  ...
  _updateLayerStatus(list: Displayable[]) {
    ...
    for (i = 0; i < list.length; i++) {
      ...
      if (el.__dirty & REDRAW_BIT && !el.__inHover) {
        // Ignore dirty elements in hover layer.
        layer.__dirty = true
   ...
   */
  getRenderedCanvas(opts?: { backgroundColor?: string | GradientObject | ImagePatternObject; pixelRatio?: number }) {
    opts = opts || {}
