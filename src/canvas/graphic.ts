...
function brushPath(ctx: CanvasRenderingContext2D, el: Path, style: PathStyleProps, inBatch: boolean) {
  ...
  if (!inBatch) {
    ...
    if (hasFillGradient) {
      ...
      if (fillPattern) {
        // createCanvasPattern may return false if image is not ready.
...
function isTransformChanged(m0: MatrixArray, m1: MatrixArray): boolean {
  if (m0 && m1) {
    return (
      m0[0] !== m1[0] || m0[1] !== m1[1] || m0[2] !== m1[2] || m0[3] !== m1[3] || m0[4] !== m1[4] || m0[5] !== m1[5]
    )
  } else if (!m0 && !m1) {
    // All identity matrix.
    return false
...
export function brush(ctx: CanvasRenderingContext2D, el: Displayable, scope: BrushScope, isLast: boolean) {
  const m = el.transform
...
function brushIncremental(ctx: CanvasRenderingContext2D, el: IncrementalDisplayable, scope: BrushScope) {
