...
const DEFAULT_ANIMATABLE_MAP = reduce(
  TRANSFORMABLE_PROPS,
  (obj, key) => {
    obj[key] = true
    return obj
...
class Element<Props extends ElementProps = ElementProps> {
  ...
  updateInnerText(forceUpdate?: boolean) {
    ...
    if (textEl && (!textEl.ignore || forceUpdate)) {
      ...
      const isInside =
        textConfig.inside == null // Force to be inside or not.
  ...
  protected _applyStateObj(
    ...
    if (state && state.textConfig) {
      ...
      if (normalState.textConfig) {
        // Only restore if changed and saved.
  ...
  protected static initDefaultProps = (function () {
    ...
    function createLegacyProperty(key: string, privateKey: string, xKey: string, yKey: string) {
...
function animateToShallow<T>(
  ...
  if (
    ...
    const animator = new Animator(
      ...
      additive
        ? filter(
            ...
            existsAnimators,
            animator => animator.targetName === topKey
          )
        : null
    )
    ...
    animator
      .whenWithKeys(duration == null ? 500 : duration, reverse ? reversedTarget : target, animationKeys)
      .delay(delay || 0)
