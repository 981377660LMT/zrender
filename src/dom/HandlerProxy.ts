// 应该是个中间层，监听 viewport dom 的鼠标事件， 对事件进行规范化处理，然后再次出发给 Handler 类；避免在 handler 中需要做各种判断。
// 用于实现DOM事件代理，所有画布内元素的事件都是从画布DOM的事件进行代理进入
/* global document */
...
function isLocalEl(instance: HandlerDomProxy, el: Node) {
  ...
  while (
    elTmp &&
    elTmp.nodeType !== 9 &&
    !(isLocal = (elTmp as DomExtended).domBelongToZr || (elTmp !== el && elTmp === instance.painterRoot))
  ) {
...
class DOMHandlerScope {
  ...
  constructor(domTarget: HTMLElement | HTMLDocument, domHandlers: DomHandlersMap) {
